...
const csvPath = path.resolve(__dirname, '../../../ClaudeCode/youtube-analytics/raw_analytics.csv');
...
}).filter(v => v.video_id && v.views > 0);

// Output as JSON for seeding
const output = JSON.stringify({ videos }, null, 2);
const outPath = path.resolve(__dirname, '../frontend/public/sample-analytics.json');
fs.writeFileSync(outPath, output);
console.log(`Wrote ${videos.length} videos to ${outPath}`);

// Also write a minimal version for dev
const devPath = path.resolve(__dirname, '../frontend/src/data/sampleAnalytics.json');
const dir = path.dirname(devPath);
if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
fs.writeFileSync(devPath, output);
console.log(`Wrote dev sample data to ${devPath}`);
