...
export async function migrateProfilePicture(): Promise<void> {
  try {
    // Check if column already exists
    const checkColumn = isSqlite()
      ? "SELECT COUNT(*) as count FROM pragma_table_info('users') WHERE name='profile_picture'"
      : "SELECT COUNT(*) as count FROM information_schema.columns WHERE table_name='users' AND column_name='profile_picture'";
