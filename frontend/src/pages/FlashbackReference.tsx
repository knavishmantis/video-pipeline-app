...
export default function FlashbackReference() {
  ...
  useEffect(() => {
    const loadGuide = async () => {
      try {
        const response = await fetch('/flashback-formula.md');
        if (response.ok) {
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-neutral-600">This reference guide is only available to clippers.</p>
        </div>
      </div>
    );
  }
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-12 h-12 rounded-full bg-orange-100 flex items-center justify-center">
              <IconCamera className="h-6 w-6 text-orange-600" />
            </div>
            <div>
              <h1 className="text-3xl font-bold text-neutral-900 mb-2">Flashback Reference</h1>
              <p className="text-neutral-600">
                Complete guide on how clips should be styled and formatted
              </p>
            </div>
          </div>
          ...
          <div className="p-4 bg-orange-50 border-2 border-orange-300 rounded-lg">
            <p className="text-sm text-orange-900 font-semibold">
              ⚠️ This is your primary reference document for clip styling and formatting. 
              Refer to this guide regularly when creating clips to ensure consistency and quality.
            </p>
          </div>
        </div>
